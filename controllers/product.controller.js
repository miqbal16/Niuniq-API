...
exports.getProducts = asyncHandler(async (req, res, next) => {
  ...
  if (req.params.storeId) {
    ...
    if (!store) {
      return next(
        new ErrorResponse(
          `Store not found with id of ${req.params.storeId}`,
          404,
        ),
      );
    }
    ...
    total = await Product.countDocuments(JSON.parse(queryStr));
  }
...
exports.addProduct = asyncHandler(async (req, res, next) => {
  ...
  for (let i = 0; i < upload.images.length; i += 1) {
    if (
      upload.images[i].mimetype !== 'image/jpeg'
      && upload.images[i].mimetype !== 'image/jpg'
      && upload.images[i].mimetype !== 'image/png'
    ) {
      return next(
        new ErrorResponse(
          'The file photo must be in jpeg, jpg, or png format',
          400,
        ),
      );
    }
  ...
  for (let i = 0; i < upload.images.length; i += 1) {
    if (upload.images[i].size > process.env.MAX_FILE_PHOTO) {
      return next(
        new ErrorResponse(
          `Uploaded photos cannot be more than ${
            process.env.MAX_FILE_PHOTO / 1000000
          } mb`,
          400,
        ),
      );
  ...
  // const photoDir = `${req.protocol}://${req.get(
  //   'host',
  // )}/documents/images/products/`;
  // const qrCodeDir = `${req.protocol}://${req.get(
  //   'host',
  // )}/documents/images/QRcodes/`;

  // Save photo ke folder public
  for (let i = 0; i < upload.images.length; i += 1) {
    upload.images[i].mv(
      `${process.env.FILE_UPLOAD_PHOTO_PRODUCT_PATH}/${upload.images[i].name}`,
      async (err) => {
        if (err) {
          await product.remove();
          console.error(err);
    ...
    fileImage.push(upload.images[i].name);
  }
  ...
  const url = `http://www.niuniq.id/detailview/${product.productId}`;
  const QRcodeFileName = `qrcode_${product._id}.png`;
...
exports.deleteProduct = asyncHandler(async (req, res, next) => {
  ...
  for (let i = 0; i < product.photos.length; i += 1) {
    fs.unlinkSync(
      `public/documents/images/products/photo_${product._id}_${i + 1}.jpg`,
    );
  }
...
// @desc      Varification Product
// @route     PUT /api/web/niuniq/products/verification/:id
// @access    Private (Need Auth)
exports.verificationProduct = asyncHandler(async (req, res, next) => {
  const product = await Product.findByIdAndUpdate(
    req.params.id,
    { isVerification: req.body.isVerification, verificationMessage: req.body.verificationMessage },
    {
      new: true,
      runValidators: true,
    },
