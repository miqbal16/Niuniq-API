...
exports.updateUser = asyncHandler(async (req, res, next) => {
  ...
  if (!store) {
    return next(
      new ErrorResponse(`User with id ${req.params.id} not have market`, 404),
    );
  }
  ...
  if (store) {
    for (let i = 0; i < store.products.length; i += 1) {
      for (let j = 0; j < store.products[i].photos.length; j += 1) {
        fs.unlinkSync(
          `${process.env.FILE_UPLOAD_PHOTO_PRODUCT_PATH}photo_${
            store.products[i]._id
          }_${j + 1}.jpg`,
        );
      }
      ...
      if (store.products[i].qrCode) {
        fs.unlinkSync(
          `${process.env.FILE_UPLOAD_QRCODE_PRODUCT_PATH}qrcode_${store.products[i]._id}.png`,
        );
      }
    }
    ...
    if (store.photo) {
      fs.unlinkSync(
        `${process.env.FILE_UPLOAD_PHOTO_STORE_PATH}photo_${store._id}.jpg`,
      );
    }
    ...
    if (store.logo) {
      fs.unlinkSync(
        `${process.env.FILE_UPLOAD_LOGO_STORE_PATH}logo_${store._id}.png`,
      );
    }
  }
